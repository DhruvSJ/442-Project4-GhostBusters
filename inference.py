...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for item in self.items():
            seq.append(item[0])
            weights.append(float(item[1])/float(self.total()))
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            ...
            currentB = self.beliefs[pos]
            getProb = self.getObservationProb(observation,currentPos,pos,jail)
            newBelief = currentB * getProb
    ...
    def elapseTime(self, gameState):
        ...
        for oldpos in ghostPos:
            ...
            for newDis in ghostPos:
                newProb = newDist[newDis]
                distrib[newDis] += newProb * self.beliefs[oldpos]

        distrib.normalize()
        self.beliefs = distrib
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            value = self.getObservationProb(observation,state,particle,jail)
            distrib[particle] += value
        ...
        if distrib.total() != 0:
            ...
            for i in range(self.numParticles):
                self.particles.append(distrib.sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        parti = (itertools.product(self.legalPositions, repeat = self.numGhosts))
        position = list(parti)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(ghosts):
                prob *= self.getObservationProb(observation[i], current ,particle[i], self.getJailPosition(i))
